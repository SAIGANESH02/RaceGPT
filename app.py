...
def build_prompt(context: str, question: str, race_name: str, driver_name: str, file_name: str) -> str:
    """
    ...
    prompt = (
        ...
        f"Question: {question}\n"
        "Answer:\n"
        f"Cited from: Race: {race_name}, Driver: {driver_name}, File: {file_name}"
    )
    return prompt
...
async def handle_message(message: cl.Message):
    ...
    if combined_conversations:
        ...
        for conv in combined_conversations:
            ...
            citations.append(f"Race: {conv['race_name']}, Driver: {conv['driver_name']}, File: {conv['file_name']}")

        # Extract details for citation from the first relevant conversation
        race_name = combined_conversations[0]['race_name']
        driver_name = combined_conversations[0]['driver_name']
        file_name = combined_conversations[0]['file_name']

        # Build the prompt with citation
        prompt = build_prompt(context, user_query, race_name, driver_name, file_name)
        ...
        try:
            ...
            # Structure the response with citations
